...
while screen == "newGame":
    ...
    highscoreimg = Image(Point(rulesRect.getCenter().getX(), 525), "culmimages/highscore.gif")
    ...
    while screen == "title":
        ...
        if 385 >= click.getX() >= 215 and 240 >= click.getY() >= 220:  # if user clicks on classic
            ...
            # While we were working on the game,
            # this was the grid that we used to help visualize where the blocks were. You can uncomment to see the grid:
                ...
                while placing:
                    ...
                    while time.time() < timeEnd:  # time for input
                        ...
                        if key == hotkey:
                            ...
                            if (prevPosition - position >= size * 2 or position - prevPosition >= size * 2) and addBlock is False:
